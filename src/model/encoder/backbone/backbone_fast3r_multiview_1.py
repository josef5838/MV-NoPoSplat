...
class Fast3rMulti(CroCoNet):
    ...
    def _decoder(self, feat, pose, extra_embed=None):
        ...
        for view_idx in range(v):            
            for batch_idx in range(b):      
                num_patches = feat[batch_idx, view_idx].shape[0] 
                image_ids.extend([view_idx] * num_patches)     

        image_ids = torch.tensor(image_ids).reshape(b, -1).to(feat.device)


        # project to decoder dim
        f = rearrange(feat, "b v l c -> (b v) l c") #[4, 257, 1024]
        f = self.decoder_embed(f) #[4, 257, 1024]
        f = rearrange(f, "(b v) l c -> b v l c", b=b, v=v) #[1, 4, 257, 1024]
        final_output.append(f.shape)

        x = rearrange(f, 'b v l c -> b (v l) c')
        pose = rearrange(pose, 'b v l c -> b (v l) c')
        ...
        for blk in self.dec_blocks:
            x = blk(x, pose) # [1, 514, 768]

            f = rearrange(x, 'b (v l) c -> b v l c',v=v,l=l)
    ...
    def forward(self,
        ...
        feat, pose, _ = self._encode_image(images_all, shape_all, intrinsic_embedding_all) # [4, 257, 1024], [4, 257, 2]

        feat = rearrange(feat, "(b v) l c -> b v l c", b=b, v=v) # [1, 4, 257, 1024]
        pose = rearrange(pose, "(b v) l c -> b v l c", b=b, v=v) # [1, 4, 257, 2]
