...
class BackboneFast3r(CroCoNet):
    ...
    def __init__(self, cfg: BackboneFast3rCfg, d_in: int) -> None:
        ...
        #     # self.dec_blocks2 = deepcopy(self.dec_blocks_nopo)  # This is used in DUSt3R and MASt3R
        ...
        self.register_buffer(
            "image_idx_emb",
            torch.from_numpy(
                get_1d_sincos_pos_embed_from_grid(self.dec_embed_dim, np.arange(1000)) # self.dec_embed_dim
            ).float(),
            persistent=False,
        )
    ...
    def _set_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec):
        ...
        # self.liner = nn.Linear(dec_embed_dim_fast, dec_embed_dim, bias=True)
        # transformer for the decoder
        # self.dec_blocks_nopo = nn.ModuleList([
        #     DecoderBlock(1024, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
        #     for i in range(3)])
    ...
    def _encode_image(self, image, true_shape, intrinsics_embed=None):
        ...
        #image: 2, 3, 256, 256 -> number, channel size size
        #true_shape: 2 2 -> number, size
        x, pos = self.patch_embed(image, true_shape=true_shape) #x: [2, 256, 1024] pos:[2, 256, 2]
        ...
        if intrinsics_embed is not None:
            if self.intrinsics_embed_type == 'linear':
                ...
                x = torch.cat((x, intrinsics_embed), dim=1) #[2, 257, 1024]
                add_pose = pos[:, 0:1, :].clone()
                add_pose[:, :, 0] += (pos[:, -1, 0].unsqueeze(-1) + 1)
                pos = torch.cat((pos, add_pose), dim=1) # [2, 257, 2]
    ...
    def _encode_symmetrized(self, view1, view2, force_asym=False):
        img1 = view1['img'] # B     [b c image_size image_size]
        img2 = view2['img'] #1 3 256 256 [b c image_size image_size]

        B = img1.shape[0]
        # Recover true_shape when available, otherwise assume that the img shape is the true one
        shape1 = view1.get('true_shape', torch.tensor(img1.shape[-2:])[None].repeat(B, 1)) #[1, 2]: [[256, 256]] 
        shape2 = view2.get('true_shape', torch.tensor(img2.shape[-2:])[None].repeat(B, 1)) # B 2
        # warning! maybe the images have different portrait/landscape orientations

        intrinsics_embed1 = view1.get('intrinsics_embed', None) # B 1 1024
    ...
    def _decoder_fast3r(self, f1, pos1, f2, pos2):
        ...
        image_pos = image_idx_emb[image_ids] #[1, 514, 768]
        # print("image_pos is: ", image_pos)
    ...
    def forward(self,
        ...
        if self.intrinsics_embed_loc == 'encoder' and (self.intrinsics_embed_type == 'token' or self.intrinsics_embed_type == 'linear'):
            intrinsic_embedding = self.intrinsic_encoder(context["intrinsics"].flatten(2)) # 1 2 9 -> 1 2 1024 
            view1['intrinsics_embed'] = intrinsic_embedding[:, 0].unsqueeze(1) # B 1 1024
            view2['intrinsics_embed'] = intrinsic_embedding[:, 1].unsqueeze(1) # B 1 1024

        if symmetrize_batch:
            instance_list_view1, instance_list_view2 = [0 for _ in range(b)], [1 for _ in range(b)]
            view1['instance'] = instance_list_view1 #[0] 1xb
            view2['instance'] = instance_list_view2 #[1] 1xb
            view1['idx'] = instance_list_view1 #[0] 1xb
            view2['idx'] = instance_list_view2 #[1] 1xb
            ...
            # encode the two images --> B,S,D
            #shape1: [1, 2]; feat1:[1, 257, 1024]; pos:[1, 257, 2]
